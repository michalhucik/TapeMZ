/**
 * @file   test_fsk_roundtrip.c
 * @brief  Round-trip test: raw data -> FSK WAV -> wav_decode_fsk -> data
 *
 * Testovaci program:
 * 1. Vytvori testovaci data (256 bajtu: 0x00-0xFF)
 * 2. Zakoduje je jako FSK signal pres mzcmt_fsk (speed 0, raw - bez tape wrapperu)
 * 3. Ulozi do WAV souboru
 * 4. Nacte WAV zpet, preprocessing, extrakce pulzu
 * 5. Zavola wav_decode_fsk_decode_bytes() primo
 * 6. Porovna dekodovane bajty s originalem
 *
 * FSK format:
 * - 8 cyklu na bajt, MSB first, bez stop bitu
 * - Bit 1 = long cyklus, bit 0 = short cyklus
 * - 7 rychlostnich urovni (speed 0-6)
 */
...
#include "libs/wav_analyzer/wav_decode_fsk.h"
...
int main ( int argc, char *argv[] ) {
    ...
    const char *wav_path = "temp/fsk_test.wav";
    ...
    /* === 2. FSK kodovani (raw, speed 0) === */

    st_CMT_STREAM *stream = mzcmt_fsk_create_stream (
                                body, 256, MZCMT_FSK_SPEED_0,
                                CMT_STREAM_TYPE_VSTREAM, rate
                            );
    if ( !stream ) {
        fprintf ( stderr, "ERROR: mzcmt_fsk_create_stream failed\n" );
        return 1;
    }

    printf ( "FSK stream created: %.3f sec (speed 0)\n",
    ...
    aerr = wav_decode_fsk_decode_bytes ( &seq, 0, &decoded_data, &decoded_size, &consumed );
    wav_pulse_sequence_destroy ( &seq );

    if ( aerr != WAV_ANALYZER_OK ) {
        fprintf ( stderr, "ERROR: wav_decode_fsk_decode_bytes failed: %s\n",
                  wav_analyzer_error_string ( aerr ) );
        return 1;
    }

    printf ( "FSK decoded: %u bytes\n", decoded_size );
    ...
    if ( compare_data ( "FSK data", body, decoded_data, cmp_size ) ) {
        failures++;
