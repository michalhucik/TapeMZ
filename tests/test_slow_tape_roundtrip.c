/**
 * @file   test_slow_tape_roundtrip.c
 * @brief  Round-trip test: MZF -> SLOW tape WAV -> wav_analyzer -> MZF
 *
 * Testovaci program pro dvourychlostni SLOW tape signal:
 * 1. Vytvori testovaci MZF v pameti (telo 256B: 0x00-0xFF)
 * 2. Zakoduje jako kompletni tape signal (preloader + loader + SLOW data)
 *    pres mzcmt_slow_create_tape_stream
...
int main ( int argc, char *argv[] ) {
    ...
    const char *wav_path = "temp/slow_tape_test.wav";
    ...
    header.ftype = 0x01;
    memcpy ( header.fname.name, "TEST SLOW TAPE\r", 15 );
    header.fsize = 256;
    header.fstrt = 0x1200;
    header.fexec = 0x1200;
    ...
    st_MZCMT_SLOW_TAPE_CONFIG config = {
        .pulseset = MZCMT_SLOW_PULSESET_800,
        .speed = MZCMT_SLOW_SPEED_0,
        .loader_speed = 0,
    };

    st_CMT_STREAM *stream = mzcmt_slow_create_tape_stream (
                                &header, body, header.fsize, &config,
                                CMT_STREAM_TYPE_VSTREAM, rate
                            );
    if ( !stream ) {
        fprintf ( stderr, "ERROR: mzcmt_slow_create_tape_stream failed\n" );
        return 1;
    }

    printf ( "SLOW tape stream created: %.3f sec (speed 0)\n",
    ...
    if ( f->format != WAV_TAPE_FORMAT_SLOW ) {
        printf ( "FAIL: Expected format SLOW, got %s\n", wav_tape_format_name ( f->format ) );
        failures++;
    } else {
        printf ( "OK: Format = SLOW\n" );
    }
