/**
 * @file   test_tc_loader_all.c
 * @brief  Integracni test: dekodovani TurboCopy TURBO loader nahravky
 *
 * WAV soubor tc-loader-all.wav obsahuje 5 kopii programu Turbo Copy V1.21
 * nahrane s TURBO loaderem v rychlostech 1:1, 2:1, 7:3, 8:3, 3:1.
 *
 * Kazda TURBO nahravka se sklada z:
 * - NORMAL FM preloader (hlavicka + turbo loader binarni kod)
 * - TURBO FM datova cast (uzivatelska data)
 *
 * V soucasne dobe wav_analyzer dekoduje pouze NORMAL FM casti.
 * Kopie nahrana rychlosti 1:1 ma NORMAL casovani, takze jeji preloader
 * obsahuje kompletni program a je plne dekodovatelna.
 *
 * Test overuje:
 * 1. Alespon 1 soubor dekodovan (kopie 1:1)
 * 2. Prvni dekodovany soubor ma spravna data (body = referencni MZF)
...
static const char *g_wav_path = "../tstdata/tc-loader-all.wav";
 ...
 *  5 kopii: 1x NORMAL (1:1) + 4x TURBO (2:1, 7:3, 8:3, 3:1).
 */
#define EXPECTED_MIN_FILES  5
...
int main ( int argc, char *argv[] ) {
    ...
    printf ( "=== TurboCopy TURBO Loader Test ===\n" );
    printf ( "WAV: %s\n", wav_path );
    printf ( "REF: %s\n", ref_path );
